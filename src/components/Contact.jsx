 ...
 const validateField = (name, value) => {
  if (name === 'phone') {
    ...
    if (!phoneNumber || !phoneNumber.isValid()) {
      return 'Please enter a valid phone number including country code (e.g., +1 415 123 4567)';
    }
    return '';
  }
  ...
  switch (name) {
    case 'name':
      ...
      if (!regexRules.name.test(value)) return 'Name should be 3-50 alphabetic characters';
      break;
    case 'email':
      if (!value.trim()) return 'Email is required';
      if (!regexRules.email.test(value)) return 'Please enter a valid email address (e.g., user@example.com)';
      break;
    case 'message':
      if (!value.trim()) return 'Message is required';
      if (!regexRules.message.test(value)) return 'Message should be between 10 and 500 characters';
      break;
    default:
      return '';
  }
