...
export default function TeamSection() {
  const scrollRef = useRef(null);
  const cardRef = useRef(null); // Reference to the first card

  const scrollLeft = () => {
    const cardWidth = cardRef.current?.offsetWidth || 325;
        ...
        >
          {teamMembers.map((member, idx) => (
            <div
              ...
              style={{ maxWidth: '100%' }}
              key={idx}
              ref={idx === 0 ? cardRef : null} // Assign the ref only to the first card
            >
              <div className="position-relative mb-4">
                <img
                  src={member.img}
